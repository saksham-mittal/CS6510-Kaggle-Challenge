...
labelEnc = LabelEncoder()
male = labelEnc.fit_transform(training_labels)
oneHotEnc = OneHotEncoder(categorical_features=[0])
training_labels = oneHotEnc.fit_transform(male.reshape(-1, 1)).toarray()
print(male)

classifier = Sequential()

# Adding the input layer and the first hidden layer
classifier.add(Dense(output_dim = 12, init = 'uniform', input_dim = training_set.shape[1]))
...
classifier.add(Activation('relu'))
classifier.add(BatchNormalization())
classifier.add(Dense(output_dim = 5, init = 'uniform'))
classifier.add(Activation('relu'))
classifier.add(BatchNormalization())
...
classifier.fit(training_set, training_labels, batch_size = 64, nb_epoch = 10)

out = classifier.predict(test_set)

out = np.argmax(out, axis = 1) + 1

with open("output_neural.csv", "w") as fp:
    fp.write("id,pricing_category\n")
    for i in range(test_id.shape[0]):
        fp.write("{},{}.0\n".format(test_id[i], out[i]))
